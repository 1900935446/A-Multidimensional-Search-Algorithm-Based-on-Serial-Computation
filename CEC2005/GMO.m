...
% GMO: Geometric Mean Optimizer                                                                 
function [z_final,pos_final,z_iter] = GMO(np,maxit,varmin,varmax,nx,fobj)
...
varmax=varmax.*ones(1,nx);
varmin=varmin.*ones(1,nx);
...
z_pbest=zeros(1,np);
z_kbest=zeros(1,np);
z_optimal=inf*ones(1,maxit);
pos_final=zeros(nx,1);
...
[pp,pv]=GMOInitialization(np,nx,varmax,varmin,velmax,velmin);
